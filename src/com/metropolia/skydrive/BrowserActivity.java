...
import com.metropolia.skydrive.dialogs.DownloadDialog;
import com.metropolia.skydrive.dialogs.NewFolderDialog;
import com.metropolia.skydrive.dialogs.RenameDialog;
import com.metropolia.skydrive.dialogs.SharingDialog;
...
/**
 * This acts as the main activity after sign-in. This class allows
 * user to download, upload, camera capture, traverse from Home etc.
 */
public class BrowserActivity extends SherlockListActivity
{
    ...
    /*
     * Holder for the ActionMode, part of the contextual action bar
    ...
    {
        if (startIntent.getAction() != null && startIntent.getAction().equalsIgnoreCase("metropolia.skydrive.UPLOAD_PICK_FOLDER"))
        {
    ...
    private void handleListItemClick(AdapterView<?> parent, final int position)
    {
        ...
        skyDriveObj.accept(new SkyDriveObject.Visitor()
        {
            ...
            public void visit(SkyDrivePhoto photo)
            {
                ...
                startPhotoDialog.putExtra("metropolia.skydrive.PHOTO_ID", photo.getId());
                startPhotoDialog.putExtra("metropolia.skydrive.PHOTO_NAME", photo.getName());
                if (!connectionIsUnavailable())
                {
    ...
    public boolean onOptionsItemSelected(MenuItem item)
    {
        switch (item.getItemId())
        {
            ...
            case R.id.newFolder:
                ...
                startNewFolderDialog.putExtra("metropolia.skydrive.CURRENT_FOLDER", currentFolderId);
                if (!connectionIsUnavailable())
                {
            ...
            case R.id.camera:
                //cameraCapture();
            	startActivity(new Intent(getApplicationContext(), CameraCaptureActivity.class));
            	//Toast.makeText(getApplicationContext(), "Camera capture.....yet to implement...", Toast.LENGTH_SHORT).show();
            	//System.out.println("Camera capture.....yet to implement...sysout");
                return true;
            case R.id.reload:
                reloadFolder();
                return true;
